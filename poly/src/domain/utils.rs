...
pub(crate) fn parallel_fft<T: DomainCoeff<F>, F: FftField>(
    ...
    tmp.par_iter_mut()
        ...
        .for_each(|(k, kth_poly_coeffs)| {
            ...
            // O(|coset|log(|coset|)). If this understanding is the case, its
            // not doing standard Cooley-Tukey. At the moment, this has time
            // complexity of at least 2*N field mul's per thread, so we will be
            // getting pretty bad parallelism. Exact complexity per thread atm
            // is `2N + (N/num threads)log(N/num threads)` field muls Compare to
            // the time complexity of serial is Nlog(N) field muls), with log(N) in [15, 25]
